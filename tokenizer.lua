...
local keyword = {}
for _, value in ipairs(tokentypes.keywords) do
    keyword[value] = true
end
...
function tokenizer.tokenize(code)
    ...
    while i <= #code do
        ...
        if rest:match("^%-%-%[%[") then
            text = rest:match("^%-%-%[%[.-%]%]")
            if not text then
                error("unterminated multiline comment at line " .. line)
            end
            advance(text)
        ...
        elseif rest:match("^<<") then
            text = rest:match("^(<<.->>)")
            if not text then
                error("unterminated raw Lua block at line " .. line)
            end
            add("rawlua", text:sub(3, -3), at_line, at_column)
            advance(text)
        ...
        elseif rest:sub(1, 1) == "\n" or rest:sub(1, 1) == ";" then
            ...
            if #tokens == 0 or tokens[#tokens].type ~= "newline" then
                add("newline", text, at_line, at_column)
            end
            advance(text)
        elseif rest:match("^%d+%.?%d*") then
            text = rest:match("^%d+%.?%d*")
            add("number", text, at_line, at_column)
            advance(text)
        elseif rest:match("^[%a_][%w_]*") then
            text = rest:match("^[%a_][%w_]*")
            add(keyword[text] and "keyword" or "identifier", text, at_line, at_column)
            advance(text)
        elseif rest:match('^"') or rest:match("^'") then
            local quote, j = rest:sub(1, 1), 2
            while j <= #rest and rest:sub(j, j) ~= quote do
                if rest:sub(j, j) == "\\" then
                    j = j + 1
                end
                j = j + 1
            end
            ...
            text = rest:sub(1, j)
            add("string", text, at_line, at_column)
            advance(text)
        elseif
            rest:match("^==")
            or rest:match("^~=")
        ...
        then
            text = rest:match("^==")
                ...
                or rest:match("^%.%.")
                or rest:match("^[+%-%*/%%%=<>]")
            add("operator", text, at_line, at_column)
            advance(text)
        elseif string.find("(){}[],:\\?.", rest:sub(1, 1), 1, true) then
            text = rest:sub(1, 1)
            add("symbol", text, at_line, at_column)
            advance(text)
        else
            error(
